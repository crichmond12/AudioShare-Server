...
The `task` field is parsed into `commands::Task` and routed by `commands::dispatch()` (called from `Connection::handle_task()`). Recognized tasks: `play`, `pause`, `stop`, `seek`, `volume`, `list_outputs`. Anything else is `Unknown`. (`list_outputs` is special-cased in `handle_task` — not `dispatch` — because it needs the connection to push; see the `outputs` message below.)
