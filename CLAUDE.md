...
The `task` field is parsed into `commands::Task` and routed by `commands::dispatch()` (called from `Connection::handle_task()`). Recognized tasks: `play`, `pause`, `stop`, `seek`, `volume`, `list_outputs`, `list_zones`, `create_zone`, `delete_zone`, `rename_zone`, `set_zone_outputs`. Anything else is `Unknown`. (`list_outputs` and `list_zones` are special-cased in `handle_task` — not `dispatch` — because they need the connection to push; see the push messages below.)
