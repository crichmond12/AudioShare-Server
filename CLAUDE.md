...
The `task` field is parsed into `commands::Task` and routed by `commands::dispatch()` (called from `Connection::handle_task()`). Recognized tasks: `play`, `pause`, `stop`, `seek`, `volume`, `list_outputs`, `list_zones`, `create_zone`, `delete_zone`, `rename_zone`, `set_zone_outputs`, `list_sources`, `get_art`, `reroute`. Anything else is `Unknown`. (`list_outputs`, `list_zones`, and `list_sources` are special-cased in `handle_task` — not `dispatch` — because they need the connection to push; see the push messages below.)
...
```
`source` is the home zone's id (stable handle for that receiver); `dest_zone` is the current routing target (the home zone by default; moves on reroute). `active` is `true` when a session is live. `routed` is `true` when the source's PCM is being written to a sink (versus connected-but-unrouted). `art_id` is the SHA-256 hex of the current album art (empty when none); fetch the bytes by reference via the `get_art` task (art is never inlined in the push). Track fields are cleared when the session ends. `SOURCES_CHANGED` is a `tokio::sync::broadcast` parallel to `OUTPUTS_CHANGED`; connections subscribe to both.
...
**Reroute a live AirPlay session (`reroute`) — iOS → server.** Moves an active source's audio to a different zone while the phone stays connected to the same receiver (the receiver's AirPlay/Bonjour name does **not** change — the app's `dest_zone` is the source of truth for where it's playing):
...
Reroute is a per-session override: `dest_zone` reverts to the source's home zone when the session ends. Errors: `missing_source`, `missing_zone`, `unknown_source` (no such source or no active session), and the `play`-style routing codes `unknown_zone` / `zone_has_no_outputs` / `mixed_zone_unsupported` / `no_free_stream` (reroute resolves the target sink eagerly through the same `zone_sink()` path, so a failure leaves the source unchanged).
